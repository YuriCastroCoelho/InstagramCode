...
# ---- Classe da Árvore Binária de Busca para Recomendação ----

class NoRecomendacao:
    def __init__(self, reels_id, score):
        self.reels_id = reels_id
        self.score = score
        self.esquerda = None
        self.direita = None

class ArvoreRecomendacao:
    def __init__(self):
        self.raiz = None

    def inserir(self, reels_id, score):
        self.raiz = self._inserir_recursivo(self.raiz, reels_id, score)

    def _inserir_recursivo(self, no, reels_id, score):
        if no is None:
            return NoRecomendacao(reels_id, score)
        if score < no.score:
            no.esquerda = self._inserir_recursivo(no.esquerda, reels_id, score)
        else:
            no.direita = self._inserir_recursivo(no.direita, reels_id, score)
        return no

    def em_ordem(self):
        resultado = []
        self._em_ordem_recursivo(self.raiz, resultado)
        return resultado[::-1]  # ordem decrescente

    def _em_ordem_recursivo(self, no, resultado):
        if no is not None:
            self._em_ordem_recursivo(no.esquerda, resultado)
            resultado.append((no.reels_id, no.score))
            self._em_ordem_recursivo(no.direita, resultado)

# ---- Aplicativo principal com Tkinter ----

class ReelsArvore:
    def __init__(self, root):
        self.root = root
        self.root.title("Sistema de Recomendação de Reels com Árvore")
    ...
    def recomendar(self):
        ...
        arvore = ArvoreRecomendacao()

        for reels, quem_assistiu in self.reels_assistidos.items():
            score = sum(1 for pessoa in quem_assistiu if pessoa in seguindo)
            if score > 0:
                arvore.inserir(reels, score)

        recomendados = arvore.em_ordem()
        if recomendados:
            texto = "Reels sugeridos: " + ", ".join(f"{r} (score {s})" for r, s in recomendados)
        else:
